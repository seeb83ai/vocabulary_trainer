...
test.describe('Quiz – acknowledged words (main user)', () => {
  ...
  test('play button is inside result green box after answering (issue #169)', async ({ page }) => {
    await useZhToTranslMode(page);
    await page.goto('/train');
    await expect(page.locator('#card-area')).toBeVisible({ timeout: 12_000 });
    ...
    // The play button must be inside the word-breakdown (green box), not standalone
    await expect(page.locator('#word-breakdown .btn-result-play')).toBeVisible();
    // The standalone result-play-btn must remain hidden
    await expect(page.locator('#result-play-btn')).not.toBeVisible();
  });
...
test.describe('Quiz – Chinese character size and play button (issue #158)', () => {
  ...
  test('play button is hidden in transl_to_zh mode (issue #168)', async ({ page }) => {
