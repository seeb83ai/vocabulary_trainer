		...
		fn: func(db *sql.DB) error {
			...
			if !strings.Contains(wordsSql, "'de'") {
				for _, stmt := range []string{
					...
					`INSERT OR IGNORE INTO words_de (id, text, language, pinyin, created_at, needs_review)
					 SELECT id, text, language, pinyin, created_at, COALESCE(needs_review, 0) FROM words`,
					`DROP TABLE words`,
					`ALTER TABLE words_de RENAME TO words`,
					`CREATE INDEX IF NOT EXISTS idx_words_text_lang ON words(text, language)`,
				} {
			...
			// German translations table (mirrors translations but for DE words).
			for _, stmt := range []string{
				`CREATE TABLE IF NOT EXISTS de_translations (
				  de_word_id INTEGER NOT NULL REFERENCES words(id) ON DELETE CASCADE,
				  zh_word_id INTEGER NOT NULL REFERENCES words(id) ON DELETE CASCADE,
				  PRIMARY KEY (de_word_id, zh_word_id)
				)`,
				`CREATE INDEX IF NOT EXISTS idx_de_translations_zh ON de_translations(zh_word_id)`,
			} {
				if _, err := db.Exec(stmt); err != nil {
					return fmt.Errorf("create de_translations: %w", err)
				}
			}
			return nil
		},
