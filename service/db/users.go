...
func (s *Store) GetUserSettingsRaw(ctx context.Context, userID int64) (
	...
	err = s.db.QueryRowContext(ctx, `
		SELECT primary_lang, secondary_lang,
		       ...
		       COALESCE(extend_session_with_extra_words, 1),
		       ...
		       COALESCE(baseline_new_bucket_enabled, 0),
		       COALESCE(baseline_new_bucket_value, 10),
		       ...
		       COALESCE(gamification_hide_pinyin_from_bucket, '70-84'),
		       COALESCE(match_game_pinyin_reveal, 'always'),
		       COALESCE(match_game_sm2_update, 'always'),
		...
		&st.ExtendSessionWithExtraWords,
		...
		&st.BaselineNewBucketEnabled,
		&st.BaselineNewBucketValue,
		...
		&st.GamificationHidePinyinFromBucket,
		&st.MatchGamePinyinReveal,
		&st.MatchGameSM2Update,
		&st.WrongAnswerRetryMode,
...
func (s *Store) UpdateUserSettings(ctx context.Context, userID int64, st models.UserSettings) error {
	...
	_, err := s.db.ExecContext(ctx, `
		UPDATE user_settings SET
			...
			extend_session_with_extra_words = ?,
			...
			baseline_new_bucket_enabled     = ?,
			baseline_new_bucket_value       = ?,
			...
			gamification_hide_pinyin_from_bucket = ?,
			match_game_pinyin_reveal        = ?,
			match_game_sm2_update           = ?,
