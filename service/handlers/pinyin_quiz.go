...
func (h *PinyinQuizHandler) Next(w http.ResponseWriter, r *http.Request) {
	...
	sound, progress, err := h.Store.GetNextPinyinCard(r.Context(), int64(1), tags, skipNew)
	if err != nil {
	...
	if progress.TotalAttempts == 0 {
		_ = h.Store.AcknowledgePinyinSound(r.Context(), int64(1), sound.ID)
	}
...
func (h *PinyinQuizHandler) Answer(w http.ResponseWriter, r *http.Request) {
	...
	progress, err := h.Store.GetPinyinProgress(r.Context(), int64(1), req.SoundID)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
	...
	if err := h.Store.UpdatePinyinProgress(r.Context(), int64(1), updated); err != nil {
	...
	_ = h.Store.RecordPinyinDailyStat(r.Context(), int64(1), correct, sound.Tone)
	...
	if !correct && confusedWithID > 0 {
		_ = h.Store.UpsertPinyinConfusion(r.Context(), int64(1), sound.ID, confusedWithID)
...
func (h *PinyinQuizHandler) Stats(w http.ResponseWriter, r *http.Request) {
	...
	due, total, err := h.Store.GetPinyinStats(r.Context(), int64(1), tags)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
func (h *PinyinQuizHandler) DailyStats(w http.ResponseWriter, r *http.Request) {
	stats, err := h.Store.GetPinyinDailyStatsHistory(r.Context(), int64(1))
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
