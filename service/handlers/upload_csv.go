...
func (h *UploadCSVHandler) UploadCSV(w http.ResponseWriter, r *http.Request) {
	...
	hasPinyinCol := strings.EqualFold(strings.TrimSpace(header[1]), "pinyin")
	langStart := 1
	if hasPinyinCol {
		langStart = 2
	}
	if len(header) <= langStart {
		writeError(w, http.StatusBadRequest, "CSV must have at least one language column")
		return
	}
