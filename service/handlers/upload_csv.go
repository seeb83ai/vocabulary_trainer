...
func (h *UploadCSVHandler) UploadCSV(w http.ResponseWriter, r *http.Request) {
	...
	if len(header) < 2 {
		writeError(w, http.StatusBadRequest, "CSV must have at least 2 columns: chinese, <lang>")
		return
	}
	...
	hasPinyin := strings.EqualFold(strings.TrimSpace(header[1]), "pinyin")
	langStart := 1
	if hasPinyin {
		langStart = 2
		if len(header) < 3 {
			writeError(w, http.StatusBadRequest, "CSV with pinyin column must have at least 3 columns: chinese, pinyin, <lang>")
			return
		}
	}
