...
async function submitAnswer(e) {
  ...
  try {
    ...
    const correctBox = `
      <div class="p-3 bg-green-50 border border-green-200 rounded-xl">
        ...
        <div class="flex items-center gap-2">
          ...
          <button class="btn-result-play text-2xl text-gray-400 hover:text-blue-500 transition leading-none shrink-0" type="button" title="Read aloud">🔊</button>
        </div>
    ...
    if (!result.correct) {
      ...
      const resultPlayInline = breakdown.querySelector('.btn-result-play');
      if (resultPlayInline) resultPlayInline.addEventListener('click', () => playAudio(currentCard.word_id, result.zh_text));
      show('word-breakdown');
