...
describe('matchGameOutcome', () => {
  it('is correct when the right box is the word\'s own translation', () => {
    expect(matchGameOutcome(0, 0, 'können', 'in der Lage sein', ['in der Lage sein', 'können'], new Set())).toBe('correct');
  });

  it('is wrong when the right text is not among the word\'s translations', () => {
    expect(matchGameOutcome(1, 0, 'möglicherweise', 'in der Lage sein', ['in der Lage sein', 'können'], new Set())).toBe('wrong');
  });
  ...
  it('is blocked when a shared translation is claimed but its true owner still needs it (issue #215)', () => {
    ...
    expect(matchGameOutcome(1, 0, 'können', 'in der Lage sein', ['in der Lage sein', 'können'], new Set())).toBe('blocked');
  });
  ...
  it('is correct via shared translation once the true owner is already matched', () => {
    ...
    expect(matchGameOutcome(1, 0, 'können', 'in der Lage sein', ['in der Lage sein', 'können'], new Set([1]))).toBe('correct');
  });

  it('swaps boxes when the right box shows exactly the text of the word\'s own box (issue #473)', () => {
    // 帮忙 (lIdx=0) and 帮助 (rightIdx=1) both show "Hilfe": either box is
    // fine for either word, even while 帮助 is not matched yet.
