...
- **Look-alike characters.** Some characters look almost the same, for example 囗 (wéi, enclosure) and 口 (kǒu, mouth). When pinyin is hidden, you cannot tell them apart from the glyph. For these characters, the quiz card shows a hint below the prompt, for example "≠ 口 (mouth)". The hint shows on component cards and on word cards that show the Chinese text as the prompt. It does not show in *Translation → Chinese* or voice-only modes, because there it would give the answer. The pairs are in the `lookalike_chars` table (one row for each direction). The default pairs are 囗/口, 土/士, 未/末, 已/巳 and 日/曰. To add a pair, insert two rows into this table.
