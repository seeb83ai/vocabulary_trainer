...
- **Bucket growth indicator** — the result screen shows a row of growth icons (🌰🌱🌿🌳🌸) marking which accuracy tier (New/Struggling/Learning/Practicing/Mastered) a word, HMM entity, or component is in, with the current tier highlighted. **Celebrate bucket changes** (optional setting, off by default, Settings → Training Mode → Quiz Display) shows a full-screen "Level up!" interstitial between the result and the next card whenever a tier advances
