...
- **Gamification.** Enable a word-matching mini-game that appears during training. Configure how often, in minutes, the game may interrupt training. When the game triggers, it shows a Chinese column and a shuffled translation column. Each translation box shows only its first meaning (or its first two, if both are short) rather than a word's full dictionary gloss, and skips example-sentence/measure-word entries (`Bsp.:`/`CL:`/`ZEW:`) the same way training does. Click a Chinese word, then its translation, to match them. Correct pairs turn green, and wrong pairs flash red. If a word shares its translation text with another still-unmatched word, claiming that word's box alone does not complete the match. Instead, the box flashes yellow, so you cannot claim it out from under the word it actually belongs to. Exception: when two boxes show exactly the same text (for example 帮忙 and 帮助, both "Hilfe"), either box is correct for either word. The game updates SM-2 progress for each matched word.
