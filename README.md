...
- **Retype on wrong answer.** This optional 3-way setting is off by default, in Settings → Training Mode → Wrong Answer Retry: **Off** continues immediately; **Only what was tested** requires retyping only the field(s) the card's direction actually asked for (e.g. just the translation for a Chinese → translation card, just the Chinese word for a translation → Chinese card); **Chinese and translation** always requires both, regardless of direction. Whichever field(s) are required, the "Next" button (and the "Accept as correct" / "Add as translation" shortcuts, which would otherwise skip the retype) stay disabled until they check out. This mirrors the same required-retype pattern used for the new-word introduction screen.
