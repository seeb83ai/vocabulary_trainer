...
- **Come-back-tomorrow summary.** The "All done for today!" screen shows your current day streak (consecutive training days) and how many reviews come due tomorrow, so the spaced-repetition loop is visible from day one. When at least one word moved into a better proficiency bucket today, the screen also shows how many — computed by comparing today's bucket snapshot in `daily_stats` against the closest prior day's snapshot and summing the positive deltas across the struggling/learning/practicing/mastered buckets (the "new" bucket is excluded, since growth there reflects new words being added, not existing ones improving).
...
- **Retype on wrong answer.** This optional 3-way setting is off by default, in Settings → Training Mode → Wrong Answer Retry: **Off** continues immediately; **Only what was tested** requires retyping only the field(s) the card's direction actually asked for (e.g. just the translation for a Chinese → translation card, just the Chinese word for a translation → Chinese card); **Chinese and translation** always requires both, regardless of direction. Whichever field(s) are required, the "Next" button stays disabled until they check out. The "Accept as correct" and "Add as translation" shortcuts stay available alongside the retype fields and, like "Next" when the gate is off, still advance straight to the next card when clicked — accepting the answer already confirms it, so no retype is required in that case. This mirrors the same required-retype pattern used for the new-word introduction screen.
...
- **Translation ranking.** An optional setting in Settings → Training Mode → Quiz Display ("Hide rare translations during training") — preselected for newly registered users, with max translations shown defaulting to 4 (existing users keep whatever they already had, off by default). CC-CEDICT/HanDeDict-derived translations follow the order of the dictionary: first the first gloss of every sense (the `/`-separated parts of an entry), then the second gloss of every sense, and so on. Example sentences and measure-word notes (`Bsp.:`/`CL:`/`ZEW:`) come after the real meanings. For 对 (HanDeDict `/gegenüber (P), entgegengesetzt (Adj)/korrekt; richtig (Adj)/an; zu; für (P)/…/`) this gives gegenüber, korrekt, an, antworten, behandeln, …. Within this order, a short gloss still comes before a long one (15 characters or more). Only a dictionary gloss that is not in the word's current dictionary entry is ordered by how common its rarest word is, using bundled English/German word-frequency lists (from [hermitdave/FrequencyWords](https://github.com/hermitdave/FrequencyWords), same source as the existing Chinese frequency list). When enabled, only the top N translations per language (configurable) are shown directly on quiz cards; translations you added or edited yourself are prioritized into that visible set first, but still count against the cap like any other translation. Anything over the cap is collapsed into a "More info" toggle right below the visible list (both on the question screen's translation hint and on the answer-result screen), so you can still see it on demand instead of it being silently dropped — and it always stays a valid answer, cap or no cap. On the *Translation → Chinese* question screen specifically, CEDICT/HanDeDict example-sentence annotations are left out of "More info" entirely (rather than just collapsed), since an example sentence can contain the Chinese answer you're about to type; they still show in "More info" everywhere else (new-word introduction, answer-result screen). A separate "Also hide translations with no frequency data" toggle controls whether a gloss with no match in the frequency list counts as low-priority (collapsed if over the cap) or keeps its normal place — off by default. A "My own translations" dropdown (only active while ranking is on) sets where your own translations go: "Show first" (default) orders them your own → dictionary order → no frequency data → ranked; "Show last" orders them dictionary order → ranked → no frequency data → your own, so your own translations can end up in "More info" when the cap is reached. Within each language's shown list, a trailing short HanDeDict/CEDICT part-of-speech tag like `(S)` or `(V)` is hidden, and an exact duplicate gloss (case-insensitive, after that tag is hidden) is collapsed to a single entry — this is display-only and does not change the cap, the "More info" list, or which answers are accepted.
...
- Vocabulary management: add, edit, delete, search, paginate, and sort by any column. Searching ranks exact matches (Chinese text, pinyin, or a translation) above partial matches, ahead of the chosen sort. The app shows SM-2 progress per word.
...
- **Free dictionary lookup.** The "Translate" button in the Add/Edit Word form first tries a free, ungated exact-match lookup against the imported CC-CEDICT/HanDeDict data before falling back to DeepL — so it now works for every user, not just plus/admin accounts or a personal DeepL key. Each dictionary sense fills its own translation field: definitions are split at `;` and `,` (HanDeDict separates senses with commas), but not at commas inside brackets such as `(in the capacity of, as)`. Sub-words created from the dictionary use the same split, and a schema migration splits existing comma-joined dictionary translations the same way (translations you entered yourself stay unchanged).
...
- **Gamification.** Enable a word-matching mini-game that appears during training. Configure how often, in minutes, the game may interrupt training. When the game triggers, it shows a Chinese column and a shuffled translation column. Each translation box shows only its first meaning (or its first two, if both are short) rather than a word's full dictionary gloss, and skips example-sentence/measure-word entries (`Bsp.:`/`CL:`/`ZEW:`) the same way training does. Click a Chinese word, then its translation, to match them. Correct pairs turn green, and wrong pairs flash red. If a word shares its translation text with another still-unmatched word, claiming that word's box alone does not complete the match. Instead, the box flashes yellow, so you cannot claim it out from under the word it actually belongs to. The game updates SM-2 progress for each matched word.
  ...
  A **"Hide pinyin hint once a word reaches"** setting (default **Practicing**) picks the minimum accuracy bucket — New, Struggling, Learning, Practicing, or Mastered, the same buckets used elsewhere in the app — at and above which a word's tile stops showing its pinyin under the hanzi up front, so the game gets harder to rely on rote pinyin recall as you improve. Words below the chosen bucket keep showing pinyin immediately, as before, unaffected by the setting below.
...
Like `import-hanzi`, this is a manual, one-time (or re-run-on-update) operational step against `data/vocab.db` — it is not run automatically at startup. Each run replaces all existing entries of its language, so running it again never creates duplicates.

The import keeps the `/` borders between the senses of an entry. Translation ranking needs these borders to show the first gloss of every sense first. A database imported before this change has no borders: there, every gloss counts as its own sense, which gives plain dictionary order. Run both imports again once to get the sense order.
...
| `GET` | `/api/quiz/stats` | Get due-today and total card counts (`tags` query param); includes `difficult_remaining` (flagged drill words still to answer) and `words_improved_today` (words that moved into a better proficiency bucket today vs. the closest prior day's snapshot) |
...
| `GET` | `/api/components/coverage` | List every hanzi component appearing in the user's current zh vocabulary with the zh word IDs it covers, plus which of them already have a component_progress row (`trained_characters`) — used by the Settings page to preview how many components a candidate coverage-target percentage would select for training, how many are already in training, and how many of those would be out of scope at that target |
